...
public class SendEmailActivity extends AppCompatActivity implements IFirebaseLoadDoneDiscipline {
    ...
    public void allStudentsEmail() {

        DatabaseReference disciplineRef = databaseDisciplineReference
                .child("disciplines")
                .child(idUserLogged);
        ...
        disciplineRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    Discipline discipline = snapshot.getValue(Discipline.class);
                    idDiscipline = discipline.getIdDiscipline();

                    DatabaseReference studentsInDisciplineRef = disciplineRef.child("students");

                    studentsInDisciplineRef.addValueEventListener(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            for (DataSnapshot snap : dataSnapshot.getChildren()) {
                                ...
                                if (student.getStudentDelegate().equals("YES")) {
                                    allDelegatesEmailList.add(studentEmail);
                                    sendEmail(studentEmail);
                                }
                            }
                        }
